...
start.date.end.date1 <- start.date.assigned %>%
  left_join(end.date.assigned.ID) %>% 
  filter(!is.na(Rx_End_Date))
...
treatment.info.pf <- pf.missing.assigned %>% 
  mutate(Prj_ID = FID_PrescribedFires003 + max(treatment.info.001$Prj_ID)) %>% 
...
write_csv(treatment.info.pf,
          file = "data/versions-from-R/06_prescribed-fires-added-treatment-info_v003.csv")


save.image("RData/06_prescribed-fires-added_v003.RData")
